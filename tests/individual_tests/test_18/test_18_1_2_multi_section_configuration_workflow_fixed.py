...
def test_18_1_2_multi_section_configuration_workflow(
    ...
    try:
        ...
        except Exception as cleanup_process_error:
            logger.warning(
                f"{device_model}: Cleanup process failed: {cleanup_process_error}"
            )
            print(
                f"Warning: Cleanup process failed but test passed for {device_model}: {cleanup_process_error}"
            )
