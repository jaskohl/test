"""
Test 3.1.1: Identifier Field Persistence - Pure Page Object Pattern
...
def test_3_1_1_identifier_field_persistence(
    general_config_page: GeneralConfigPage, request
):
    """
    Test 3.1.1: Identifier Field Persistence - Pure Page Object Pattern

    Purpose: Verify device identifier field persistence using pure page object methods
    ...
    logger.info(f"Testing identifier field persistence on {device_model}")
    ...
    try:
        ...
        try:
            if hasattr(general_page, "configure_device_info"):
                ...
                logger.info(f"Using basic identifier field configuration")
                config_success = (
            ...
            if config_success:
                logger.info(
                    f"Identifier field configuration attempted on {device_model}"
                )
            else:
                logger.warning(
                    f"Identifier field configuration failed on {device_model}"
                )
            ...
            if device_series == 3:
                ...
                try:
                    if hasattr(general_page, "verify_identifier_field_visible"):
                        ...
                        # Fallback: check page data for identifier field
                        if page_data and "identifier" in str(page_data).lower():
                            logger.info(
                                f"Identifier field found in page data on Series 3 device"
                            )
                            persistence_validated = True
                            print(
                                f"IDENTIFIER FIELD DETECTED: {device_model} (Series 3)"
                            )
                ...
                try:
                    if hasattr(general_page, "verify_identifier_field_visible"):
                        general_page.verify_identifier_field_visible()
                        logger.info(
                            f"Identifier field accessible on Series 2 device {device_model}"
                        )
                        persistence_validated = True
                        print(f"IDENTIFIER FIELD ACCESSIBLE: {device_model} (Series 2)")
                    else:
                        logger.warning(
                            f"Identifier field persistence failed: expected '{test_value}', got '{persisted_value}'"
                        )
                except Exception as e:
        ...
        try:
            ...
            if hasattr(general_page, "test_identifier_field_editing"):
                try:
                    general_page.test_identifier_field_editing()
                    logger.info(f"Identifier field editing tested using page object")
                except Exception as e:
        ...
        if persistence_validated:
            ...
            pytest.fail(
                f"Identifier field persistence validation failed on {device_model}"
            )

    except Exception as e:
        logger.error(f"Identifier field persistence test failed on {device_model}: {e}")
        raise
    ...
    logger.info(f"Identifier field persistence test completed for {device_model}")
