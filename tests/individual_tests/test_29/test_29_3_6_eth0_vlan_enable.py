"""
Test 29.3.6 Eth0 VLAN Enable - Pure Page Object Pattern
Category: 29 - Network Configuration Series 3
Purpose: Test eth0 VLAN enable/disable checkbox functionality.
Expected: VLAN enable checkbox visible and enabled for Series 3.
...
def test_29_3_6_eth0_vlan_enable(unlocked_config_page: Page, base_url: str, request):
    """
    Test 29.3.6: eth0 VLAN Enable Configuration - Pure Page Object Pattern
    ...
    logger.info(f"Testing eth0 VLAN enable on {device_model}")
    ...
    try:
        vlan_locator = network_page.get_eth0_vlan_enable_field_locator()

        if vlan_locator and vlan_locator.count() > 0:
            expect(vlan_locator).to_be_visible(timeout=device_timeout)
            expect(vlan_locator).to_be_enabled(timeout=device_timeout)
            logger.info(f"eth0 VLAN enable checkbox validated - visible and enabled")
            print(f"ETH0 VLAN ENABLE VALIDATED: {device_model}")
        else:
            logger.info(
                f"VLAN enable field not visible for eth0 on {device_model} (expected on some configurations)"
            )
            print(f"ETH0 VLAN ENABLE NOT PRESENT: {device_model}")

    except Exception as e:
        pytest.fail(
            f"eth0 VLAN enable checkbox validation failed on {device_model}: {e}"
        )
    ...
    logger.info(f"eth0 VLAN enable test completed for {device_model}: {device_info}")

    # Additional validation - get eth0 VLAN configuration through page object
    try:
        eth0_vlan_config = network_page.get_eth0_vlan_configuration()
        if eth0_vlan_config:
            logger.info(
                f"eth0 VLAN configuration retrieved through page object: {eth0_vlan_config}"
            )
        else:
            logger.info(
                f"No eth0 VLAN configuration retrieved through page object for {device_model}"
            )
    except Exception as e:
        logger.warning(
            f"eth0 VLAN configuration retrieval failed for {device_model}: {e}"
        )
