"""
Category 27: PTP Configuration - Test 27.20.2
Utility Profile Timing Configuration - Pure Page Object Pattern
Test Count: 13 of 25 in Category 27
...
def test_27_20_2_utility_profile_timing_configuration(
    ...
    """
    Test 27.20.2: Utility Profile Timing Configuration - Pure Page Object Pattern
    ...
    Series: Series 3 only - validates Utility Profile timing configuration workflow
    """
    ...
    try:
        ...
        domain_configured = ptp_config_page.configure_domain_number(port, 50)
        assert domain_configured, f"Should configure domain number for {port}"

        priority_configured = ptp_config_page.configure_priorities(port, 100, 200)
        assert priority_configured, f"Should configure priorities for {port}"
        ...
        domain_persisted = persisted_data.get("domain_number") == "50"
        priority1_persisted = persisted_data.get("priority_1") == "100"
        priority2_persisted = persisted_data.get("priority_2") == "200"

        assert domain_persisted, f"Domain number should persist for {port}"
        assert priority1_persisted, f"Priority 1 should persist for {port}"
        assert priority2_persisted, f"Priority 2 should persist for {port}"

        if domain_persisted and priority1_persisted and priority2_persisted:
            logger.info(f"Utility Profile configuration persisted for {port}")
        else:
            logger.warning(
                f"Utility Profile configuration may not have persisted for {port}"
            )

    except Exception as e:
        logger.warning(
            f"Utility Profile timing configuration testing failed for port {port}: {e}"
        )
    ...
    print(
        f"UTILITY PROFILE TIMING CONFIGURATION VALIDATED: {device_model} (Series {device_series})"
    )
