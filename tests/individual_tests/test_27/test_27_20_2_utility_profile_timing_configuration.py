"""
Test 27.20.2: IEC 61850-9-3 Utility Profile Timing Configuration
Purpose: Verify timing intervals are readonly in Utility Profile, only Domain/Priority editable
Series: Series 3 Only
Device Behavior: Only Domain number, Priority 1, and Priority 2 are editable in Utility Profile
Based on test_27_ptp_config.py line 643 - MODERNIZED v3.0
"""
...
def test_27_20_2_utility_profile_timing_configuration(
    ...
    assert page_data.get("priority_1") == "100", f"Priority 1 should persist for {port}"
    assert page_data.get("priority_2") == "200", f"Priority 2 should persist for {port}"
