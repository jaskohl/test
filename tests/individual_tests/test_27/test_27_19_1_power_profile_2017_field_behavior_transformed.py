"""
Category 27: PTP Configuration - Test 27.19.1
Power Profile 2017 Field Behavior - Pure Page Object Pattern
Test Count: 8 of 25 in Category 27
...
def test_27_19_1_power_profile_2017_field_behavior(
    ...
    """
    Test 27.19.1: Power Profile 2017 Field Behavior - Pure Page Object Pattern
    Purpose: Verify all fields editable in Power Profile 2017 with device-aware validation
    Expected: All PTP fields remain editable in UI (constraints applied server-side)
    Series: Series 3 only - validates Power Profile 2017 field behavior patterns
    """
    ...
    try:
        ...
        assert (
            profile_configured
        ), f"Should successfully select Power Profile 2017 for {port}"

        # PURE PAGE OBJECT PATTERN: Use page object method to validate field editability
        field_behavior_valid = (
            ptp_config_page.validate_power_profile_2017_field_behavior(port)
        )

        if field_behavior_valid:
            logger.info(f"Power Profile 2017 field behavior validated for {port}")
        else:
            logger.warning(
                f"Power Profile 2017 field behavior validation failed for {port}"
            )
        ...
        advanced_timing_editable = (
            ptp_config_page.validate_advanced_timing_fields_editable(port)
        )
        if advanced_timing_editable:
            logger.info(f"Advanced timing fields remain editable for {port}")
        else:
            logger.warning(f"Advanced timing fields may not be editable for {port}")
        ...
        if save_success:
            logger.info(f"Power Profile 2017 configuration saved for {port}")
        else:
            logger.warning(f"Power Profile 2017 configuration save failed for {port}")

    except Exception as e:
        logger.warning(
            f"Power Profile 2017 field behavior testing failed for port {port}: {e}"
        )
    ...
    print(
        f"POWER PROFILE 2017 FIELD BEHAVIOR VALIDATED: {device_model} (Series {device_series})"
    )
