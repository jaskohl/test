"""
Category 27: PTP Configuration - Test 27.19.2
Power Profile 2017 Priority Configuration - Pure Page Object Pattern
Test Count: 9 of 25 in Category 27
...
def test_27_19_2_power_profile_2017_priority_configuration(
    ...
    """
    Test 27.19.2: Power Profile 2017 Priority Configuration - Pure Page Object Pattern
    Purpose: Verify priority configuration works in Power Profile 2017 with device-aware validation
    Expected: Priority configuration persists after save operation
    Series: Series 3 only - validates Power Profile 2017 priority configuration workflow
    """
    ...
    try:
        ...
        priority_configured = ptp_config_page.configure_priorities(port, 100, 110)
        assert (
            priority_configured
        ), f"Should successfully configure priorities for {port}"
        ...
        priority2_persisted = persisted_data.get("priority_2") == "110"

        assert priority1_persisted, f"Priority 1 should persist after save for {port}"
        assert priority2_persisted, f"Priority 2 should persist after save for {port}"

        if priority1_persisted and priority2_persisted:
            logger.info(f"Priority configuration persisted for {port}")
        else:
            logger.warning(f"Priority configuration may not have persisted for {port}")
    ...
    print(
        f"POWER PROFILE 2017 PRIORITY CONFIGURATION VALIDATED: {device_model} (Series {device_series})"
    )
