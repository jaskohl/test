"""
Category 27: PTP Configuration - Test 27.49
Default UDP Profile eth4 - Pure Page Object Pattern
Test Count: 22 of 25 in Category 27
...
- Encapsulates Default UDP Profile eth4 field behavior validation within page object layer
"""
...
def test_27_49_default_udp_profile_eth4(
    ...
    """
    Test 27.49: Default UDP Profile eth4 - Pure Page Object Pattern
    Purpose: Verify UDP transport auto-selection in Default UDP Profile on eth4 with device-aware validation
    Expected: Network transport automatically set to UDPv4 in Default UDP Profile
    Series: Series 3 only - validates Default UDP Profile field behavior on specific interface
    """
    ...
    logger.info(
        f"Testing Default UDP Profile on {target_port} for {device_model} with {timeout_multiplier}x timeout multiplier"
    )
    ...
    try:
        ...
        profile_configured = ptp_config_page.configure_ptp_profile(
            target_port, "Default Profile (UDPv4)"
        )
        assert (
            profile_configured
        ), f"Should successfully select Default UDP Profile for {target_port}"
        ...
        if udp_transport_valid:
            logger.info(f"UDP transport auto-selection validated for {target_port}")
        else:
            logger.warning(
                f"UDP transport auto-selection validation failed for {target_port}"
            )

        # Test specific field behaviors with page object methods
        network_transport_correct = (
            ptp_config_page.validate_network_transport_auto_selection(
                target_port, "UDPv4"
            )
        )
        if network_transport_correct:
            logger.info(f"Network transport correctly set to UDPv4 for {target_port}")
        else:
            logger.warning(
                f"Network transport may not be correctly set to UDPv4 for {target_port}"
            )

        # Test UDP TTL field behavior
        udp_ttl_editable = ptp_config_page.validate_udp_ttl_field_editable(target_port)
        if udp_ttl_editable:
            logger.info(f"UDP TTL field is editable for {target_port}")
        else:
            logger.warning(f"UDP TTL field may not be editable for {target_port}")

        # Test save configuration
        save_success = ptp_config_page.save_port_configuration(target_port)
        if save_success:
            logger.info(f"Default UDP Profile configuration saved for {target_port}")
        else:
            logger.warning(
                f"Default UDP Profile configuration save failed for {target_port}"
            )

    except Exception as e:
        logger.warning(
            f"Default UDP Profile eth4 field behavior testing failed for port {target_port}: {e}"
        )
    ...
    logger.info(
        f"Default UDP Profile eth4 field behavior test completed for {device_model}"
    )
    ...
    print(
        f"DEFAULT UDP PROFILE ETH4 FIELD BEHAVIOR VALIDATED: {device_model} (Series {device_series})"
    )
