"""
Test: 7.2.1 - Output 2 with Available Signals [DEVICE ENHANCED]
...
def test_7_2_1_output_2_available_signals_device_enhanced(
    outputs_config_page: OutputsConfigPage, request
):
    """
    Test 7.2.1: Output 2 Available Signals [DEVICE ENHANCED]
    Purpose: Test output 2 with all available signal types for device
    Expected: Device-specific signals work correctly for output 2
    Series: Both 2 and 3
    ...
    output_2_signals = device_capabilities.get_output_signal_types(device_model, 2)

    if not output_2_signals:
        pytest.skip(f"No signals defined for output 2 on {device_model}")

    print(f"Device {device_model}: Testing output 2 with signals: {output_2_signals}")
    ...
    for signal_type in output_2_signals:
        ...
        try:
            ...
            # Select signal type for output 2
            signal_select = outputs_config_page.page.locator("select[name='signal2']")
            expect(signal_select).to_be_visible()

            signal_select.select_option(value=signal_type)
            print(f"Device {device_model}: Selected {signal_type} for output 2")
            ...
            for time_ref in ["UTC", "LOCAL"]:
                time_radio = outputs_config_page.page.locator(
                    f"input[name='time2'][value='{time_ref}']"
                )
            ...
            assert success, f"Failed to save output 2 {signal_type} configuration"
    ...
    if device_network_config and "management_interface" in device_network_config:
        ...
        print(
            f"Device {device_model} (Series {device_series}): Output 2 signal testing completed"
        )
        print(f"Management interface: {mgmt_iface}, Tested signals: {output_2_signals}")
