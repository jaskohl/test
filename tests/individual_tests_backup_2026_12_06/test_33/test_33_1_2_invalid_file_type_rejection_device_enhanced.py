"""
Test 33 1 2 Invalid File Type Rejection (Device Enhanced)
...
ENHANCED: DeviceCapabilities integration with device-aware file validation patterns
...
def test_33_1_2_invalid_file_type_rejection_device_enhanced(
    ...
    Purpose: Verify invalid file types are rejected with appropriate error handling using device-aware patterns
    Expected: Non-accepted file types (.txt) rejected with error message or validation failure with device validation
    ENHANCED: DeviceCapabilities integration with device-aware file validation
    Series: Both Series 2 and 3
    """
    ...
    try:
        ...
        # ENHANCED: Cross-validate upload capabilities with DeviceCapabilities
        ...
        # ENHANCED: Navigate to upload page with device-aware timeout
        upload_config_page.page.goto(f"{base_url}/upload")
        time.sleep(1 * timeout_multiplier)

        # ENHANCED: Check file type validation through page object with device awareness
        if hasattr(upload_config_page, "check_file_type_validation"):
            ...
            # ENHANCED: Fallback: Try to select the file with device-aware patterns
            ...
            if file_input.count() > 0:
                ...
                # ENHANCED: Check for error messages or validation failures with device context
                error_indicators = [
        ...
        # ENHANCED: Device series-specific upload validation
        if device_series == "Series 3":
            # Series 3 may have more sophisticated upload validation
            logger.info(
                f"Series 3 device {device_model} - enhanced upload validation expected"
            )
