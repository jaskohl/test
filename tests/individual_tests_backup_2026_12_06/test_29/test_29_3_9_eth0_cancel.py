...
def test_29_3_9_eth0_cancel(
    ...
    Expected: Cancel button should be visible on Series 3 devices
    ...
    # FIXED: Expand eth0 panel before cancel button interaction (critical for Series 3 collapsible UI)
    _expand_eth0_panel(unlocked_config_page)

    # Locate and test eth0 cancel button
    cancel = unlocked_config_page.locator("button#button_cancel_port_eth0")

    if cancel.is_visible():
        expect(cancel).to_be_visible()
        print(" eth0 cancel button is visible")
    else:
        print("eth0 cancel button not visible (may be expected on some configurations)")
