...
def test_29_4_4_eth1_ntp(unlocked_config_page: Page, base_url: str, device_series: str):
    """
    Test 29 4 4 Eth1 Ntp
    Purpose: Test eth1 interface NTP enable checkbox functionality
    Expected: NTP checkbox should be visible and functional on Series 3 devices
    ...
    # Test NTP enable checkbox functionality
    ntp_checkbox = unlocked_config_page.locator("input[name='ntp_eth1']")
    if ntp_checkbox.is_visible():
        ...
        ntp_checkbox.click()
        time.sleep(0.2)
        new_state = ntp_checkbox.is_checked()

        # State should have changed
        assert initial_state != new_state, "NTP checkbox should toggle when clicked"
        print(" eth1 NTP checkbox is enabled and toggles correctly")
    else:
        print("eth1 NTP checkbox not visible (expected on some configurations)")
