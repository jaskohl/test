...
Gateway Field Device-Aware Configuration - DeviceCapabilities Enhanced
Test Count: 1 of 8 in Category 29
Hardware: Device Only
Priority: HIGH - Network interface validation
Series: Series 3 only
ENHANCED: Comprehensive DeviceCapabilities integration for multi-interface network configuration
...
def test_29_1_1_gateway_field_device_enhanced(
    unlocked_config_page: Page, base_url: str, request
):
    """
    Test 29.1.1: Gateway Field Configuration - DeviceCapabilities Enhanced
    ...
    ENHANCED: Full DeviceCapabilities integration for multi-interface network validation
    ...
    # Validate this is a Series 3 device (required for this test)
    ...
    for interface in gateway_interfaces:
        ...
        try:
            ...
            # Use device-aware field targeting
            gateway_field_selector = f"input[name='gateway_{interface}']"
            gateway_field = unlocked_config_page.locator(gateway_field_selector)

            # If interface-specific selector fails, try generic gateway field
            if gateway_field.count() == 0:
                gateway_field = unlocked_config_page.locator("input[name='gateway']")
    ...
    try:
        # Get device-aware save button information
        for interface in gateway_interfaces[:1]:  # Test first gateway interface
            save_button_info = DeviceCapabilities.get_interface_specific_save_button(
                device_model, "network_configuration", interface
            )
