"""
Test 31.6: HTTPS Enforcement Mode Compatibility (Device Enhanced)
...
ENHANCED: DeviceCapabilities integration with device-aware compatibility validation
"""
...
def test_31_6_enforce_https_mode_compatibility_device_enhanced(
    ...
    try:
        ...
        # ENHANCED: Verify device has HTTPS enforcement capability
        ...
        # ENHANCED: Verify the target mode is supported by the device with device-aware validation
        ...
        # ENHANCED: Verify the mode has a proper description with device context
        mode_info = next(
        ...
        # ENHANCED: Log device-specific compatibility information
        logger.info(
            f"HTTPS enforcement mode '{target_mode}' is supported on {device_model}: '{mode_info['text']}'"
        )

        # ENHANCED: Cross-validate with DeviceCapabilities save button patterns
        ...
        # ENHANCED: Validate series-specific behavior for HTTPS enforcement
        if device_series == "Series 3":
